...
async function updateCardFields(cardId, data) {
  await updateDoc(doc(db, "users", currentUser.uid, "cards", cardId), data);
  await loadDeck(currentDeck);
}
...
function startEdit(card) {
  ...
  $("input-front").value = card.front;
  $("input-example").value = card.example || "";
  $("input-back").value = card.back;
  $("input-tags").value = (card.tags || []).join(", ");
  if (card.image) {
    pendingImage = card.image;
    $("image-preview").src = card.image;
    $("image-preview").classList.remove("hidden");
    $("btn-remove-image").classList.remove("hidden");
  } else {
    pendingImage = null;
    $("image-preview").classList.add("hidden");
    $("btn-remove-image").classList.add("hidden");
  }
  $("btn-submit-card").textContent = "更新を保存";
...
function resetEditState() {
  ...
  $("image-preview").classList.add("hidden");
  $("btn-remove-image").classList.add("hidden");
  $("btn-submit-card").textContent = "カードを保存";
...
$("form-add").addEventListener("submit", async (e) => {
  ...
  if (editingCardId) {
    await updateCardFields(editingCardId, { front, back, example, tags, image: pendingImage || null });
    resetEditState();
    ...
    .forEach((c) => {
      ...
      li.innerHTML = `
        ...
        <div class="browse-item-meta">
          ...
          <span class="browse-item-actions">
      ...
      li.querySelector('[data-act="edit"]').addEventListener("click", () => startEdit(c));
...
document.querySelectorAll(".nav-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    if (btn.dataset.pane === "review") startReview();
      ...
      if (btn.dataset.pane === "add") resetEditState();
